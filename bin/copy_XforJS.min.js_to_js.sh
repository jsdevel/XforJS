...
#     XforJS-docs/
#
XFORJS_VERSION=$(cat $DIR_XFORJS/bin/VERSION);
DOC_VERSION_FILE=$DIR_BIN/VERSION;
DOC_PREVIOUS_VERSION_FILE=$DIR_BIN/VERSION_PREVIOUS;

newPrefix=XforJS.${XFORJS_VERSION};
latestBase=$DIR_XFORJS/build/javascript/$newPrefix;
newSrc=$latestBase.js;
newMin=$latestBase.min.js;
downloadDir=$DIR_PROJECT/downloads;

if [ -f $DOC_VERSION_FILE ];then
   DOC_VERSION=$(cat $DOC_VERSION_FILE);
fi

if [ -f $DOC_PREVIOUS_VERSION_FILE ];then
   PREVIOUS_VERSION=$(cat $DOC_PREVIOUS_VERSION_FILE);
else
   PREVIOUS_VERSION=$XFORJS_VERSION;
fi

currentChangelog=$DIR_PROJECT/json/CHANGELOG.${PREVIOUS_VERSION}.json;

if [ "$XFORJS_VERSION" != "$DOC_VERSION" ];then
   echo -n $XFORJS_VERSION > $DOC_VERSION_FILE;
   oldfile=$DIR_PROJECT/js/XforJS.${DOC_VERSION}.min.js;
   if [ -f $oldfile ];then
...
echo -n $PREVIOUS_VERSION > $DOC_PREVIOUS_VERSION_FILE;


cd $DIR_XFORJS

git --no-pager log $PREVIOUS_VERSION..HEAD --format=%B > $DIR_BIN/CHANGELOG

cd $DIR_BIN


#for the docs
cp $newMin $DIR_PROJECT/js/$newPrefix.min.js
...
if [ -f $currentChangelog ];then
   cp $currentChangelog $DIR_PROJECT/downloads/CHANGELOG.${PREVIOUS_VERSION}.json
fi

...
previousChangelog=$DIR_PROJECT/json/CHANGELOG.${PREVIOUS_VERSION}.json;

if [ "$NEXT_VERSION" != "$CURRENT_VERSION" ];then
   echo -n $NEXT_VERSION > $DOC_VERSION_FILE;
   oldfile=$DIR_PROJECT/js/XforJS.${CURRENT_VERSION}.min.js;
   if [ -f $oldfile ];then
...
if [ -f $previousChangelog ];then
   mv $previousChangelog $DIR_PROJECT/downloads/CHANGELOG.${PREVIOUS_VERSION}.json
fi
